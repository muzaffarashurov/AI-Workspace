...
- [x] 4.2 Новый Intent + Command для IV-001 ("отправь накладные") — реализовано
      вместе с 9.1 (см. ниже), т.к. пользователь предоставил реальный скрипт
...
- [x] 9.1 IV-001: Orchestrator реально запускает
      `InvoiceAutomation_Console_Mode_v4.2.2_headless.py` — сделано раньше
      графика (пользователь предоставил рабочий скрипт), см. Sprint 4.2.
      Jarvis только запускает существующий скрипт как есть (`subprocess`,
      наследование консоли) — логика самой автоматизации не переписывалась.
...
> Примечание: 4.2 и 9.1 выполнены раньше очереди (вне Sprint 3), потому что
> пользователь напрямую предоставил рабочий скрипт для IV-001. Это осознанное
> отклонение от порядка спринтов, не ошибка планирования.
